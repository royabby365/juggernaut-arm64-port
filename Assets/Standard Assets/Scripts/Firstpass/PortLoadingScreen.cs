...
/// this project). Distinct from the original game's LoadingScreen class.
/// Shown before heavy synchronous work (arena build, rig spawn) and hidden
/// once the caller is done. Callers must defer the heavy work by one frame
/// (yield return null) after Show() so the loading frame renders.
/// </summary>
public class PortLoadingScreen : MonoBehaviour
{
    private static PortLoadingScreen _instance;
    public static PortLoadingScreen Instance
    {
        get
        {
            if (_instance == null)
            {
                var go = new GameObject("__port_loading");
                DontDestroyOnLoad(go);
                _instance = go.AddComponent<PortLoadingScreen>();
            }
            return _instance;
        }
    }
    ...
    private void OnGUI()
    {
        ...
        // Animated dots (pure IMGUI — no assets needed)
